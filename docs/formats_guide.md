...
For Deadlocked, the gameplay core:
...
For Deadlocked, the art instances:
...
For Deadlocked, the gameplay mission instances (some files are different to this?):
...
The vertex table header is structured like so:
...
Note that for R&C1 the format is different. Each u16 in the above table is instead a u32.
