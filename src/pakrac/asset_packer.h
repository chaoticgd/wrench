...
void pack_asset_impl(OutputStream& dest, std::vector<u8>* header_dest, fs::file_time_type* time_dest, Asset& src, Game game, s32 hint = FMT_NO_HINT);

template <typename Range>
Range pack_asset(OutputStream& dest, Asset& src, Game game, s64 alignment, s32 hint = FMT_NO_HINT, Range* empty_range = nullptr) {
...
template <typename Range>
void pack_assets(OutputStream& dest, Range* ranges_dest, s32 count, CollectionAsset& src, Game game, s32 alignment, s32 hint = FMT_NO_HINT, Range* empty_range = nullptr) {
...
template <typename Range>
Range pack_asset_sa(OutputStream& dest, Asset& asset, Game game, s32 hint = FMT_NO_HINT) {
	return pack_asset<Range>(dest, asset, game, SECTOR_SIZE, hint);
}

template <typename Range>
void pack_assets_sa(OutputStream& dest, Range* ranges_dest, s32 count, CollectionAsset& src, Game game, s32 hint = FMT_NO_HINT) {
	for(size_t i = 0; i < count; i++) {
		if(src.has_child(i)) {
...
template <typename Range>
Range pack_compressed_asset(OutputStream& dest, Asset& src, Game game, s64 alignment, const char* muffin, s32 hint = FMT_NO_HINT) {
	dest.seek(dest.size());
	dest.pad(alignment, 0);
	std::vector<u8> bytes;
...
template <typename Range>
void pack_compressed_assets(OutputStream& dest, Range* ranges_dest, s32 count, CollectionAsset& src, Game game, s64 alignment, const char* muffin, s32 hint = FMT_NO_HINT) {
	for(size_t i = 0; i < count; i++) {
		if(src.has_child(i)) {
...
template <typename Range>
Range pack_compressed_asset_sa(OutputStream& dest, Asset& src, Game game, const char* muffin, s32 hint = FMT_NO_HINT) {
	return pack_compressed_asset<Range>(dest, src, game, SECTOR_SIZE, muffin, hint);
}

template <typename Range>
void pack_compressed_assets_sa(OutputStream& dest, Range* ranges_dest, s32 count, CollectionAsset& src, Game game, const char* muffin, s32 hint = FMT_NO_HINT) {
	for(size_t i = 0; i < count; i++) {
		if(src.has_child(i)) {
