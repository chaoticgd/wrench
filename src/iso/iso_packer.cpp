...
static IsoDirectory pack_globals(OutputStream& iso, std::vector<GlobalWadInfo>& globals, const AssetPackerFunc& pack_asset);
...
static void pack_level_wad(OutputStream& iso, IsoDirectory& directory, LevelWadInfo& wad, const AssetPackerFunc& pack_asset, const char* name, s32 index);
...
void pack_iso(OutputStream& iso, BuildAsset& build, Game game, const AssetPackerFunc& pack_asset) {
	...
	root_dir.subdirs.emplace_back(pack_globals(iso, toc.globals, pack_asset));
...
static IsoDirectory pack_globals(OutputStream& iso, std::vector<GlobalWadInfo>& globals, const AssetPackerFunc& pack_asset) {
	...
	for(GlobalWadInfo& global : globals) {
		...
		assert(global.asset);
		pack_asset(iso, &global.header, *global.asset);
...
static std::array<IsoDirectory, 3> pack_levels(OutputStream& iso, std::vector<LevelInfo>& levels, const AssetPackerFunc& pack_asset, Game game, s32 single_level_index) {
	...
	if(single_level_index > -1) {
		...
		for(size_t i = 0; i < levels.size(); i++) {
			...
			if(level.level) pack_level_wad(iso, levels_dir, *level.level, pack_asset, "level", i);
			if(level.audio) pack_level_wad(iso, audio_dir, *level.audio, pack_asset, "audio", i);
			if(level.scene) pack_level_wad(iso, scenes_dir, *level.scene, pack_asset, "scene", i);
		}
	} else {
...
static void pack_level_wad(OutputStream& iso, IsoDirectory& directory, LevelWadInfo& wad, const AssetPackerFunc& pack_asset, const char* name, s32 index) {
	...
	assert(wad.asset);
	pack_asset(iso, &wad.header, *wad.asset);
