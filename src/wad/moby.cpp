...
static void write_moby_submeshes(OutBuffer dest, std::vector<MobyGifUsageTableEntry>& gif_usage, s64 table_ofs, const std::vector<MobySubMesh>& submeshes);
...
MobyClassData read_moby_class(Buffer src) {
	...
	moby.anim_joints = src.read_bytes(header.anim_joints, 128, "moby anim joints");
	...
	for(auto& entry : src.read_multiple<MobySubMeshEntry>(table_ofs, count, "moby submesh table")) {
		...
		if(unpacks.size() == 3) {
			...
			for(s64 i = 0; i < texture_data.size(); i+= 0x40) {
				submesh.texture_indices.push_back(texture_data.read<s32>(i + 0x20, "moby a+d unpack"));
			}
		}
...
static void write_moby_submeshes(OutBuffer dest, std::vector<MobyGifUsageTableEntry>& gif_usage, s64 table_ofs, const std::vector<MobySubMesh>& submeshes) {
	...
	for(const MobySubMesh& submesh : submeshes) {
		...
		index_unpack.code.num = submesh.indices.size() / 4;
		...
		index_unpack.data = submesh.indices;
		write_vif_packet(dest, index_unpack);
		
		if(submesh.texture_indices.size() > 0) {
			...
			texture_unpack.code.num = submesh.texture_indices.size() * 4;
			...
			texture_unpack.code.unpack.addr = INDEX_UNPACK_ADDR_QUADWORDS + submesh.indices.size() / 4;
			u8 ad_data[0x40] = {
			...
			dest.pad(0x10);
			entry.vif_list_texture_unpack_offset = (dest.tell() - texture_unpack_ofs + 0x4) / 0x10;
		}
		dest.pad(0x10);
