...
void unpack_hud_wad(HudWadAsset& dest, InputStream& src, Game game) {
	...
	unpack_assets<BinaryAsset>(dest.online_images().switch_files(), src, ARRAY_PAIR(header.online_images), game);
	...
	unpack_compressed_assets<BinaryAsset>(dest.e3_level_ss().switch_files(), src, ARRAY_PAIR(header.e3_level_ss), game);
	unpack_compressed_asset(dest.nw_dnas_image(), src, header.nw_dnas_image, game);
	unpack_asset(dest.split_screen_texture(), src, header.split_screen_texture, game);
	unpack_assets<BinaryAsset>(dest.radar_maps().switch_files(), src, ARRAY_PAIR(header.radar_maps), game);
	unpack_assets<BinaryAsset>(dest.weapon_plates_large().switch_files(), src, ARRAY_PAIR(header.weapon_plates_large), game);
	unpack_assets<BinaryAsset>(dest.mission_plates_large().switch_files(), src, ARRAY_PAIR(header.mission_plates_large), game);
	unpack_assets<BinaryAsset>(dest.gui_plates().switch_files(), src, ARRAY_PAIR(header.gui_plates), game);
	unpack_assets<BinaryAsset>(dest.vendor_plates().switch_files(), src, ARRAY_PAIR(header.vendor_plates), game);
	unpack_asset(dest.loading_screen(), src, header.loading_screen, game);
	unpack_assets<BinaryAsset>(dest.planets().switch_files(), src, ARRAY_PAIR(header.planets), game);
	unpack_assets<BinaryAsset>(dest.cinematics().switch_files(), src, ARRAY_PAIR(header.cinematics), game);
	unpack_assets<BinaryAsset>(dest.equip_large().switch_files(), src, ARRAY_PAIR(header.equip_large), game);
	unpack_assets<BinaryAsset>(dest.equip_small().switch_files(), src, ARRAY_PAIR(header.equip_small), game);
	unpack_assets<BinaryAsset>(dest.moves().switch_files(), src, ARRAY_PAIR(header.moves), game);
	unpack_assets<BinaryAsset>(dest.save_level().switch_files(), src, ARRAY_PAIR(header.save_level), game);
	unpack_assets<BinaryAsset>(dest.save_empty().switch_files(), src, ARRAY_PAIR(header.save_empty), game);
	unpack_assets<BinaryAsset>(dest.skills().switch_files(), src, ARRAY_PAIR(header.skills), game);
	unpack_asset(dest.reward_back(), src, header.reward_back, game);
	unpack_asset(dest.complete_back(), src, header.complete_back, game);
	unpack_asset(dest.complete_back_coop(), src, header.complete_back_coop, game);
	unpack_assets<BinaryAsset>(dest.rewards().switch_files(), src, ARRAY_PAIR(header.rewards), game);
	unpack_asset(dest.leaderboard().switch_files(), src, header.leaderboard, game);
	unpack_assets<BinaryAsset>(dest.cutaways().switch_files(), src, ARRAY_PAIR(header.cutaways), game);
	unpack_assets<BinaryAsset>(dest.sketchbook().switch_files(), src, ARRAY_PAIR(header.sketchbook), game);
	unpack_assets<BinaryAsset>(dest.character_epilogues().switch_files(), src, ARRAY_PAIR(header.character_epilogues), game);
	unpack_assets<BinaryAsset>(dest.character_cards().switch_files(), src, ARRAY_PAIR(header.character_cards), game);
	unpack_asset(dest.equip_plate(), src, header.equip_plate, game);
	unpack_asset(dest.hud_flythru(), src, header.hud_flythru, game);
	unpack_assets<BinaryAsset>(dest.mp_maps().switch_files(), src, ARRAY_PAIR(header.mp_maps), game);
	unpack_assets<BinaryAsset>(dest.tourney_plates_large().switch_files(), src, ARRAY_PAIR(header.tourney_plates_large), game);
}
