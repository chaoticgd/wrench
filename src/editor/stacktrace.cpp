...
#ifdef _WIN32
// Taken from https://stackoverflow.com/questions/6205981/windows-c-stack-trace-from-a-running-app
#include <windows.h>
#include <string>
#include <sstream>
#include <vector>
...
DWORD DumpStackTrace(std::stringstream* result, EXCEPTION_POINTERS* ep);
...
DWORD DumpStackTrace(std::stringstream* result, EXCEPTION_POINTERS* ep)
{
	...
	if (!SymInitialize(process, "bin", false))
	...
	do {
		if (frame.AddrPC.Offset != 0) {
			...
			if (fnName == "RaiseException") {
				...
				*result << "No stacktrace for release builds.";
...
void generate_stacktrace_win32(std::stringstream* result) {
	__try { DebugBreak();  }
	__except (DumpStackTrace(result, GetExceptionInformation())) {}
}
...
std::string generate_stacktrace() {
	std::stringstream result;
		...
		free(strings);
	#elif defined(_WIN32)
		generate_stacktrace_win32(&result);
	#else
		result << "Taking a stacktrace on your OS is currently not supported.";
	#endif
