...
static std::vector<GameTypes> s_games;
...
int main(int argc, char** argv)
{
	...
	for (const auto& [game, path] : input_paths) {
		std::vector<u8> contents = read_file(path, true);
		
		GameTypes& game_types = games.emplace_back();
		game_types.game = game;
		parse_cpp_types(game_types.types, eat_cpp_file((char*) contents.data()));
	}
